/**
 * WhatsApp Gateway - Client A
 * Baileys instance running on port 2900
 */
...
async function initWhatsApp() {
  ...
  sock = makeWASocket({
    ...
    logger,
    browser: ['Client A Receptionist', 'Chrome', '1.0.0']
  });
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (qr) {
      ...
      alertSender.sendQRNeededAlert(CLIENT_ID, { name: 'Client A', port: PORT }, qr);
    }
...
app.listen(PORT, () => {
  logger.info(`🚀 Client A Gateway listening on port ${PORT}`);
  initWhatsApp();
});
