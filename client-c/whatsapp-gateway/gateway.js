/**
 * WhatsApp Gateway - Client C
 * Baileys instance running on port 2902
 */
...
async function initWhatsApp() {
  ...
  sock = makeWASocket({
    ...
    logger,
    browser: ['Client C Receptionist', 'Chrome', '1.0.0']
  });
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (qr) {
      ...
      alertSender.sendQRNeededAlert(CLIENT_ID, { name: 'Client C', port: PORT }, qr);
    }
...
app.listen(PORT, () => {
  logger.info(`🚀 Client C Gateway listening on port ${PORT}`);
  initWhatsApp();
});
