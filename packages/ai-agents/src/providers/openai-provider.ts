/**
 * OpenAI Provider - Simplified using base class
 */
...
export class OpenAIProvider extends BaseAIProvider {
  ...
  async generateResponse(params: GenerateResponseParams): Promise<string> {
    try {
      ...
      this.logMetrics(response.usage?.total_tokens || 0, Date.now() - startTime, 'OpenAI');
      return response.choices[0]?.message?.content || '';
    } catch (error: any) {
      this.handleError(error, 'OpenAI');
      throw new ProviderError(error.message || 'Unknown error', 'OpenAI', error.status, false);
    }
  }
  ...
  estimateCost(tokens: number): number {
    return (tokens / 1000) * 0.005; // GPT-4o pricing
  }
}
